...
from story_video.template_loader import (
    ...
    get_subtitle_style, get_publish_config,
)
...
def run_pipeline(
    ...
    Story.json must be pre-generated (by AI assistant) before calling this function.
    """
    tpl = template or {}
    ...
    # Step 1: Load story script (pre-generated by AI assistant)
    print("\n" + "=" * 60)
    print("STEP 1: Load story script")
    print("=" * 60)
    ...
    if story_path.exists():
        with open(story_path, "r", encoding="utf-8") as f:
            story = json.load(f)
        print(f"[Story] Loaded: {story_path}")
    else:
        print(f"[Story] File not found: {story_path}")
        print("[Story] story.json should be pre-generated by the AI assistant.")
        print("[Story] No text model API needed - the assistant writes story content directly.")
        raise FileNotFoundError(
            f"story.json not found at {story_path}. "
            f"Pre-generate it first, then run with --skip-story."
        )
...
Examples:
    ...
    python -m story_video.main --topic "夸父" --template mythology --skip-images
    ...
    parser.add_argument("--story-file", default="", help="指定story.json路径（必须预先由AI助手生成）")
